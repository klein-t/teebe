...
public struct ProcessGitClient: GitClient {
    ...
    public init() {
        self.queue = DispatchQueue(label: "treebranch.git", qos: .userInitiated, attributes: .concurrent)
    }
    ...
    private static func execute(_ arguments: [String], in directory: String) throws -> GitInvocationResult {
        ...
        let readQueue = DispatchQueue(label: "treebranch.git.read", attributes: .concurrent)
        group.enter()
